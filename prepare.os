...
Процедура ВыгрузитьИсходники()
	...
	Для каждого ВерсияХранилища Из ТаблицаВерсийХранилища Цикл
		...
		// ПредставлениеАвтора = "test <test@test.ru>";
			...
			ВыгрузитьОбновлениеКонфигурацииПоставщика(ВерсияХранилища.Комментарий, ПредставлениеАвтора, ВерсияХранилища.Дата, НоваяКонфигурацияПоставщика);
		Иначе
...
Процедура ВыгрузитьОбновлениеКонфигурацииПоставщика(Комментарий, ПредставлениеАвтора, Дата, НоваяКонфигурацияПоставщика);
	...
	Сообщить("Мержим изменения из master");
	...
	Сообщить("Возвращаемся в master");
...
Процедура ИнициализироватьGitsync()
	ФС.ОбеспечитьПустойКаталог(КаталогИсходниковРепозитария);
	ФС.КопироватьСодержимоеКаталога("./resources", КаталогИсходниковРепозитария);
