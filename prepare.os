...
Процедура ВыгрузитьИсходники()
	...
	Для каждого ВерсияХранилища Из ТаблицаВерсийХранилища Цикл
		...
		Если СтрокаПользователя = Неопределено Тогда
			ИмяПользователяGit = "ci-bot";
			ПочтаПользователяGit = "ci-bot@1bit.com";
		Иначе
			ИмяПользователяGit = СтрокаПользователя.ИмяПользователяGit;
			ПочтаПользователяGit = СтрокаПользователя.ПочтаПользователяGit;
		КонецЕсли;

		Сообщить("Автор: " + ИмяПользователяGit);
		...
		Если НЕ ФС.ФайлСуществует(СтараяКонфигурацияПоставщика) ИЛИ НЕ ФайлыРавны(НоваяКонфигурацияПоставщика, СтараяКонфигурацияПоставщика) Тогда
			...
			ВыгрузитьОбновлениеКонфигурацииПоставщика(ВерсияХранилища.Комментарий, ИмяПользователяGit, ПочтаПользователяGit, ВерсияХранилища.Дата, НоваяКонфигурацияПоставщика);
		Иначе
			Сообщить("Выгружаю конфигурацию разработки");
			ВыгрузитьКонфигураациюРазработчика(ВерсияХранилища.Комментарий, ИмяПользователяGit, ПочтаПользователяGit, ВерсияХранилища.Дата);
		КонецЕсли;
		// Отладка = Истина;
	КонецЦикла;
КонецПроцедуры

Процедура ВыгрузитьОбновлениеКонфигурацииПоставщика(Комментарий, ИмяПользователяGit, ПочтаПользователяGit, Дата, НоваяКонфигурацияПоставщика);
	...
	ВыгрузитьКонфигураациюРазработчика(Комментарий, ИмяПользователяGit, ПочтаПользователяGit, Дата);
...
Процедура ВыгрузитьКонфигураациюРазработчика(Комментарий, ИмяПользователяGit, ПочтаПользователяGit, Дата)
	...
	СохранитьИзмененияGit(Комментарий, ИмяПользователяGit, ПочтаПользователяGit, Дата)
КонецПроцедуры
