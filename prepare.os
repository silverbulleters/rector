...
Процедура ВыгрузитьИсходники()
	...
	Для каждого ВерсияХранилища Из ТаблицаВерсийХранилища Цикл
		...
		Если НЕ ФС.ФайлСуществует(СтараяКонфигурацияПоставщика) ИЛИ НЕ ФайлыРавны(НоваяКонфигурацияПоставщика, СтараяКонфигурацияПоставщика) Тогда
			...
			ВыгрузитьОбновлениеКонфигурацииПоставщика(ВерсияХранилища.Комментарий, ПользовательGit, ПочтаПользователяGit, ВерсияХранилища.Дата, НоваяКонфигурацияПоставщика);
		Иначе
			Сообщить("Выгружаю конфигурацию разработки");
			ВыгрузитьКонфигураациюРазработчика(ВерсияХранилища.Комментарий, ПользовательGit, ПочтаПользователяGit, ВерсияХранилища.Дата);
		КонецЕсли;
		// Отладка = Истина;
	КонецЦикла;
КонецПроцедуры
