...
Процедура ОбновитьФайлVERSION(НомерТекущегоКоммита)
	...
	НовыйТекст = СтрЗаменить("<VERSION>0</VERSION>", "<VERSION>"+ НомерТекущегоКоммита +"</VERSION>");
...
Процедура ИнициализироватьВременнуюБазу()
	...
	КонфигураторВременнойБазы.УстановитьКонтекст(ПараметрыСистемы.СтрокаПодключенияВременнойБазы,
					ПараметрыСистемы.ПользовательВременнойБазы,
					ПараметрыСистемы.ПарольВременнойБазы);
КонецПроцедуры
...
Процедура ЗаполнитьТаблицуВерсий()
	Если ТаблицаВерсийХранилища = Неопределено Тогда
		...
		КонецЕсли;
		ЛогЯдра.Информация("Выгружаю таблицу авторов храниища конфигурации");
