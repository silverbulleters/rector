...
Процедура ВыгрузитьОбновлениеКонфигурацииПоставщика(ВерсияХранилища, ПользовательGit)
	...
	Конфигуратор.ВыгрузитьКонфигурациюВФайлы(КаталогИсходниковЭкспорта);
...
Процедура ПолучитьВерсиюХранилища(ВерсияХранилища)
	ИмяФайлаКофигурации =  ОбъединитьПути(ПараметрыЭкспорта.КаталогВременныхФайлов, "export.cf");
	...
	Конфигуратор.ВыгрузитьКонфигурациюВФайлы(ПутьКВременнымИсходнымФайлам());
КонецПроцедуры
